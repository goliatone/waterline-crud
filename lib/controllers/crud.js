...
class CrudController {
    ...
    list(req, res, next) {
        ...
        //definition captures credential.user
        let relationships = helper.getResourceRelationships(this.resource);

        /*
         * We should coordinate criteria and
         * pagination parameters.
         * - have same defaults (limit)
         * - if we have a page paramter derive skip/offset
         */
        ...
        if(!criteria.limit) criteria.limit = this.paginationLimit;

        this.logger.info('list(creteria:%j).populate(%s)', criteria, relationships);
        ...
        let calls = [
            this.resource.count(),
            this.resource.find(criteria)
                .populate(relationships)
        ];

        return Promise.all(calls).then(([count, results=[]])=>{
            ...
            let payload = {
                ...
                criteria: criteria,
                count: count,
                pagination: pagination.from(req, {
                    count,
                    skip: criteria.skip,
                    //TODO: We should be able to use the same limit value
                    //across all different points...
